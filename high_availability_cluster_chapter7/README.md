...
```text
    在配置文件我们通过设置cluster-enabled选项设置为yes来启动集群功能。此外，对于每个Redis实例，在Redis集群的配置期间都会生成一个集群节点配置文件，并且每当
某写集群的信息被持久化的时候就会被修改。cluster-config-file选项用于设置配置文件的名字。(这个配置文件不可以手动修改).
    节点的含义是:如果经过了指定的超时时间就节点无响应，Redis集群节将会促发故障转移将一个从实例提升为主实例。尽管Redis集群中所有节点都需要知道对方的存在，但是
我们并不需要将CLUSTER MEET命令发生给每个节点。这个是因为，一个节点在发现另一个系欸点时会向其传播它已知的所有节点信息。为了避免混淆，我们可以让一个节点发现所有其他的
节点。这样，集群中所有的节点旧可以互相通信了。
```
